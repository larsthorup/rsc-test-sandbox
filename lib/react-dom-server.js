...
export async function renderServerAction(action) {
  const { default: serverActionFunction } = await import(`../app/${action}.js`);
  // TODO: handle action parameters
  const jsx = await serverActionFunction();
  const manifestPath = "./app/react-client-manifest.json"; // Note: relative to working directory
  const clientComponentList = JSON.parse(
    readFileSync(manifestPath, "utf8")
  );
  const jsxExpanded = await renderFromJsx(jsx, clientComponentList);
  const jsxStringified = JSON.stringify(jsxExpanded, stringifyJSX);
  return jsxStringified;
}
