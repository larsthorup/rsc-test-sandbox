      ...
      if (typeof reactNode.type === "string") {
        ...
        for (const propName in reactNode.props) {
          if (reactNode.props.hasOwnProperty(propName) && propName !== "children") {
            html += ` ${htmlPropName(propName)}="${renderProptoHtml(reactNode, propName)}"`;
          }
        }
        html += ">";
...
function injectClientScript(reactNode, clientScripts) {
  if (reactNode.type === "html" && Array.isArray(reactNode.props.children)) {
    for (const src of clientScripts) {
      reactNode.props.children.push(createElement("script", { type: "module", src }));
    }
  }
}

export async function renderServerSide(jsx, clientScripts) {
